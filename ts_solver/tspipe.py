...
def tspipe(p=8, m=7, M_BBatch=50, M_WBatch=20, T_total=10, alpha=1): 
    num_teachers = 2*(p-1)
    ops = ['F_S', 'F_T', 'B', 'W']
    delta_mem = {'F_S': M_BBatch/(m//2) + M_WBatch/(m//2), 'F_T': 0, 'B': (- M_BBatch/(m//2) - M_WBatch/(m//2)) /2, 'W': 0}
    ...
    for stage in range(1, p + 1):
        for mb in range(1, num_teachers + 1):
            T[(stage, mb, 'F_S')] = T_total / m
            T[(stage, mb, 'F_T')] = alpha * T[(stage, mb, 'F_S')]
            T[(stage, mb, 'B')] = T_total / m
            T[(stage, mb, 'W')] = T_total / m
    ...
    for mb in range(2, (num_teachers//2) + 1):
        S[(p, mb, 'F_T')] = S[(p, mb-1, 'F_T')] + T[(p, mb-1, 'F_T')]

    S[(p, 1, 'F_S')] = S[(p, num_teachers//2, 'F_T')] + T[(p, num_teachers//2, 'F_T')]
    for mb in range(2, num_teachers//2 + 1):
        S[(p, mb, 'F_S')] = S[(p, mb-1, 'F_S')] + T[(p, mb-1, 'F_S')]

    S[(p, 1, 'B')] = S[(p, num_teachers//2, 'F_S')] + T[(p, num_teachers//2, 'F_S')]
    for mb in range(2, num_teachers + 1):
        S[(p, mb, 'B')] = S[(p, mb-1, 'B')] + T[(p, mb-1, 'B')]

    S[(p, (num_teachers//2)+1, 'F_T')] = S[(p, num_teachers, 'B')] + T[(p, num_teachers, 'B')]
    for mb in range((num_teachers//2)+2, num_teachers + 1):
        S[(p, mb, 'F_T')] = S[(p, mb-1, 'F_T')] + T[(p, mb-1, 'F_T')]
    ...
    for stage in range(p-1, 1, -1):
        ...
        S[(stage, 1, 'F_S')] = S[(stage, num_teachers//2, 'F_T')] + T[(stage, num_teachers//2, 'F_T')]
        for mb in range(2, num_teachers//2 + 1):
    ...
    S[(1, 1, 'F_S')] = S[(1, num_teachers//2, 'F_T')] + T[(1, num_teachers//2, 'F_T')]
    for mb in range(2, num_teachers//2 + 1):
    ...
    S[(1, (num_teachers//2)+1, 'F_T')] = S[(1, num_teachers//2, 'F_S')] + T[(1, num_teachers//2, 'F_S')]
    for mb in range((num_teachers//2)+2, num_teachers + 1):
        S[(1, mb, 'F_T')] = S[(1, mb-1, 'F_T')] + T[(1, mb-1, 'F_T')]
    ...
    for stage in range(2, p+1):
        ...
        S[(stage, num_teachers//2+1, 'F_T')] = S[(stage-1, num_teachers//2+1, 'F_T')] + T[(stage, num_teachers//2, 'F_T')]

        if stage == p:
            S[(stage, num_teachers//2+1, 'F_T')] = S[(stage, num_teachers, 'B')] + T[(stage, num_teachers, 'B')]
        ...
        for mb in range((num_teachers//2)+2, num_teachers + 1):
            ...
            while any(
                ...
                for mb2 in range(1, num_teachers+1) if (stage, mb2, 'B') in S
            ):
                S[(stage, mb, 'F_T')] = S[(stage, num_teachers, 'B')] + T[(stage, num_teachers, 'B')]
...
if __name__ == "__main__":
    ...
    S, T, schedule, makespan, delta_mem = tspipe(p=p, m=m, M_BBatch=M_BBatch, M_WBatch=M_WBatch, T_total=T_total, alpha=alpha)
    
    plot_schedule_and_memory(S, T, schedule, makespan, delta_mem, p, m, T_total, alpha, filename_prefix="predef_tspipe_hand")
...
# # --- Bottom: Memory usage ---
# time_points = range(num_teachers * 4 + 10)
