...
def _forward(microinputs, index, model_part):
    ...
    nvtx.push_range(message=f"F{index}", color="blue", domain="tspipe", 
...
def train_tspipe(nn_deep_part, nn_light_part, global_inputs, global_targets, loss_fn, T, soft_target_loss_weight, ce_loss_weight, check, epochs, data_loader_len):
    rank = dist.get_rank()
    ...
    while True:
        ...
        if i_f < world_size - 1:
            ...
            if rank != 0:
                ...
                nvtx.push_range(message=f"m_i{i_f} from {rank-1}", color="green", domain="tspipe", 
            ...
            global_teacher_outputs[i_f] = _forward(global_inputs, i_f, nn_light_part)

            if rank != world_size - 1:
                # Send outputs to the next rank
                nvtx.push_range(message=f"m_o{i_f} to {rank+1}", color="green", domain="tspipe", 
                            category="comm", payload=rank)
                # time.sleep(0.1)
...
def tspipe(nn_deep_part, nn_light_part, data_loader, loss_fn, T, soft_target_loss_weight, ce_loss_weight):
    ...
    train_tspipe(nn_deep_part, nn_light_part, global_inputs, global_targets, loss_fn, T, soft_target_loss_weight, ce_loss_weight, check, epochs, len(data_loader))
